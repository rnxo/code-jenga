...
import { parseJsonBody, parseStartGameRequest } from "@/lib/server/validation";
...
export async function POST(request: Request, { params }: RouteParams): Promise<Response> {
  try {
    ...
    const req = parseStartGameRequest(await parseJsonBody(request));
...
async function handleStartGame(
  ...
  if (!game || !room || room.host_id !== userId) {
    throw new ApplicationError(game ? "FORBIDDEN" : "GAME_NOT_FOUND", game ? "試合開始権限がありません。" : "試合が見つかりません。");
  }
