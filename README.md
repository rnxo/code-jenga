...
| コード表示 / 編集 | Monaco Editor | ⬜ 未導入 |
| バックエンド | TypeScript（Next.js Route Handlers）| ⬜ 未実装 |
| AI | Gemini API | ⬜ 未導入 |
| コード実行 | **要再検討**（詳細は後述） | 🔴 ブロッカーあり |
| テスト | Vitest（アプリ自体のテスト用）| ⬜ 未導入 |
| DB | Supabase | 🟡 環境変数のみ設定済み |
...
- [ ] **コード実行方式**：案 A / B / C / D のどれにするか（→ 最優先）
- [ ] **対戦方式**
  - ローカル交代（1 画面で 2 人が順番に操作）… Realtime 不要で最速
  - オンライン対戦（Supabase Realtime でルーム同期）… 本命だが実装量が大きい
...
| `GEMINI_API_KEY` | Gemini API キー（**未設定**） | サーバー専用 |
